...
func (ep *Endpoint) GetDashboard(c *gin.Context) { //end point for the buyer
	defer c.Request.Body.Close()
	log.Infof("Get Dashboard Endpoint")

	var request inputDashboard //get input form model
	if err := c.ShouldBindBodyWith(&request, binding.JSON); err != nil {
		c.JSON(http.StatusBadRequest, err)
		return
	}

	log.Infof("request id : %+v", request.LoginCompanyID)
	log.Infof("request currency : %s", request.FilterCurrency)
	log.Infof("request date from: %+v", request.FilterDateFrom)
	log.Infof("request date to: %+v", request.FilterDateTo)
	//return success

	result, com, er := GetDashboard(request) //logic
	if er != nil {
		//return err
